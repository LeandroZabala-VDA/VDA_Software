...
def update_plot(frame, ser, data, line, buffer, max_points=100):
    ...
    """
    while ser.in_waiting > 0:                                            #Este while loop se ejecuta mientras haya datos disponibles en el buffer de entrada del objeto serial 
        ...
        try:
            ...
            if len(buffer) >= 5:  # Actualizar el gráfico cada 5 datos
                ...
                if len(data) > max_points:
                    data = data[-max_points:]                           # se mantiene solo los últimos max_points datos, descartando los más antiguos.
                buffer.clear()
                line.set_ydata(data)
...
def main():
    ...
    # Crear la animación
    ani = animation.FuncAnimation(fig, update_plot, fargs=(ser, data, line, buffer, max_points), init_func=init, blit=True, interval=50)
